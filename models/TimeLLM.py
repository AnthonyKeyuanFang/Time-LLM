...
class Model(nn.Module):
    ...
    def calcute_lags(self, x_enc):
        q_fft = torch.fft.rfft(x_enc.float().permute(0, 2, 1).contiguous(), dim=-1)
        k_fft = torch.fft.rfft(x_enc.float().permute(0, 2, 1).contiguous(), dim=-1)
